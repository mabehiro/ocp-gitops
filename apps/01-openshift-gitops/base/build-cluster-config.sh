...
find . -type f -path "*$CLUSTER_NAME*" -exec sed -i "s/hub-adetalhouet.rhtelco.io/$CLUSTER_NAME.$DOMAIN_NAME/g" {} +

# configure bootstrap
sed -i "s/clusters\/default/clusters\/$CLUSTER_NAME/g" bootstrap/$CLUSTER_NAME/app-of-apps.yaml

# configure cluster
#sed -i "s/cluster: default/cluster: $CLUSTER_NAME/g" clusters/$CLUSTER_NAME/applicationset.yaml
#sed -i "s/overlays\/default/overlays\/$CLUSTER_NAME/g" clusters/$CLUSTER_NAME/applicationset.yaml
